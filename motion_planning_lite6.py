...
def motion_plan_data(robot, table_id, plane_id, EXCLUDE_TABLE,robot_urdf_path, base_save_dir="dataset", yaml_config_path="config/planner_config.yaml"):
    print("\n" + "="*60)
    print("STARTING FIXED A-TO-B TEST")
    print("="*60 + "\n")

    robot.reset_posture()
    point_A_pos = [0.25, -0.2, 0.75] 
    point_B_pos = [0.35+np.random.uniform(-0.05,0), 0.2+np.random.uniform(-0.05, 0.05), 0.75+np.random.uniform(-0.05, 0.2, size=1)[0]]  # Add small random noise to B for variability
    draw_sphere(point_A_pos, radius=0.02, color=[0, 1, 0])
    draw_sphere(point_B_pos, radius=0.02, color=[1, 0, 0]) 
    # EEF looking straight down
    ...
    # 3. Spawn Multiple Adversarial Obstacles
    num_obs = p_config.get('num_obstacles', 1)
    # obstacle_ids = generate_task_obstacles_old(point_A_pos, point_B_pos, num_obs=num_obs)
    ########### NEW OBSTACLES GENERATION ############
    ...
    # 5. Get IK Solutions
    print("Solving IK for Point B (Goal)...")
    goal_config = solve_ik_collision_free(robot, planner, point_B_pos, target_orn)
    print("Solving IK for Point A (Start)...")
    start_config = solve_ik_collision_free(robot, planner, point_A_pos, target_orn)
    
    if not start_config or not goal_config:
        print("Failed to find valid IK configurations for A or B.")
        for obs_id in obstacle_ids:
            p.removeBody(obs_id)    
        return

    # Small debug: verify the TCP is actually at Point A before planning/execution
    current_tcp_pos, current_tcp_orn = robot.get_current_ee_position()
    start_pos_error = np.linalg.norm(np.array(current_tcp_pos) - np.array(point_A_pos))
    print(f"\nStart pose check:")
    print(f"  target A:  {point_A_pos}")
    print(f"  current TCP: {tuple(round(v, 4) for v in current_tcp_pos)}")
    print(f"  start error: {start_pos_error*1000:.2f} mm")

    # Reset the persistent debug trajectory so this run starts cleanly at Point A
    visualise_eef_traj.prev_pos = None
    visualise_eef_traj(current_tcp_pos)

    print("\nPlanning trajectory around wall...")
    success, path = planner.plan(start_config, goal_config, planning_time=p_config['planning_time']) 

    if success:
        # first_waypoint = np.array(path[0])
        # start_config_arr = np.array(start_config)
        # print(f"Planned path start delta: {np.max(np.abs(first_waypoint - start_config_arr)):.4f} rad")

        # # Preview the planned path directly so Point A -> Point B is visible
        # test_lite6_ompl.visualize_path(robot, path)

        # # Restore the robot back to Point A before animated execution
        # for i, joint_id in enumerate(robot.arm_controllable_joints):
        #     p.resetJointState(robot.id, joint_id, start_config[i])
        print(f"\nExecuting smoothed path...")
        collision_occurred = False # New flag
        
        for i, waypoint in enumerate(path):
            # Set motor targets
            for j, joint_id in enumerate(planner.joint_ids):
                p.setJointMotorControl2(planner.robot.id, joint_id, p.POSITION_CONTROL, 
                                        waypoint[j], 
                                        force=150.0,              # Slightly reduced max force
                                        maxVelocity=1.5,          # Slowed down for safety during testing
                                        positionGain=0.05,        # Kp: Lower means a "softer" approach to the waypoint
                                        velocityGain=1.0          # Kd: Higher adds damping to prevent overshoot/swinging
                                        )
            
            # Sub-steps for physics
            for _ in range(5):
                p.stepSimulation()
                
                # --- NEW: ACTIVE COLLISION MONITOR ---
                # Check for any contact between robot (id) and obstacles (list)
                # for obs_id in [table_id,wall_id] + obstacle_ids:
                for obs_id in [table_id] + obstacle_ids:
                    contacts = p.getContactPoints(robot.id, obs_id)
                    if len(contacts) > 0:
                        collision_occurred = True
                        print(f"💥 COLLISION DETECTED at waypoint {i}!")
                        break 
                if collision_occurred: break
                
                # Waypoint threshold check
                current = [p.getJointState(robot.id, j)[0] for j in planner.joint_ids]
                if np.max(np.abs(np.array(current) - np.array(waypoint))) < 0.01:
                    break
            
            if collision_occurred: break # Stop execution entirely
            visualise_eef_traj(p.getLinkState(robot.id, robot.eef_id)[0])

    # 7. Final Success Verification (Updated)
    ...
    for obs_id in obstacle_ids:
        p.removeBody(obs_id)
    # SUCCESS ONLY IF: Near target AND No collisions happened
    if not collision_occurred and pos_err < 0.05 and orn_err < 20:
        print("✅ TRUE SUCCESS: Goal reached without any contact.")
        return True
    else:
        reason = "Collision" if collision_occurred else "Missed Target"
        print(f"❌ FAILURE: {reason} POSITION_ERROR={pos_err*1000:.2f} mm, ORIENTATION_ERROR={orn_err:.2f} deg.")
        return False
...
def main():
    ...
    for _ in range(10):
        motion_plan_data(robot, table_id, plane_id,robot_urdf_path=robot_urdf_path, EXCLUDE_TABLE=EXCLUDE_TABLE,yaml_config_path="config/planner_config.yaml")
    p.disconnect()
